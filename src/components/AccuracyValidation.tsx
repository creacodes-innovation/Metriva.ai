...
const checks: { label: string; detail: string; value: string; status: Status }[] = [
  {
    ...
    detail: "Shoulders level, arms at 15° A-pose",
    value: "98%",
    status: "pass",
  },
  {
    label: "Full body in frame",
    detail: "Head crown to ankle visible",
    value: "OK",
    status: "pass",
  },
  {
    label: "Lighting uniformity",
    detail: "No harsh shadows on torso edge",
    value: "92%",
    status: "pass",
  },
  {
    label: "Background contrast",
    detail: "Silhouette separable from backdrop",
    value: "74%",
    status: "warn",
  },
  {
    label: "Calibration marker",
    detail: "A4 sheet detected at feet — 21.0 cm",
    value: "Locked",
    status: "pass",
  },
  ...
  {
    label: "Clothing fit",
    detail: "Loose fabric detected near waistline",
    value: "Re-take",
    status: "fail",
  },
];
              ...
              {checks.map((c) => (
                <li key={c.label} className="flex items-center gap-4 px-5 py-4">
                  <span className={`relative w-2.5 h-2.5 rounded-full ${dot[c.status]} shrink-0`}>
                    {c.status !== "pass" && (
                      <span
                        className={`absolute inset-0 rounded-full ${dot[c.status]} animate-ping opacity-60`}
                      />
                    )}
                  </span>
                  ...
                  <div className="font-mono text-xs text-muted-foreground tabular-nums w-14 text-right">
                    {c.value}
                  </div>
                  <span
                    className={`font-mono text-[10px] tracking-wider px-2 py-1 rounded border ${tag[c.status]}`}
                  >
                    {tagLabel[c.status]}
                  </span>
                </li>
          ...
          <div className="lg:col-span-5 space-y-4">
            <div className="rounded-2xl border border-amber-400/30 bg-amber-400/5 p-6">
              ...
              <p className="text-sm text-muted-foreground mt-2 leading-relaxed">
                Measurement is exported but tagged for supervisor review. Common causes: low
                contrast background, mild camera tilt, partial occlusion of one wrist.
              </p>
            </div>
            ...
            <div className="rounded-2xl border border-red-500/30 bg-red-500/5 p-6">
              ...
              <p className="text-sm text-muted-foreground mt-2 leading-relaxed">
                ...
                with a plain-English fix:{" "}
                <span className="text-foreground">
                  "Ask subject to tuck in shirt and stand straight."
                </span>
              </p>
            </div>
